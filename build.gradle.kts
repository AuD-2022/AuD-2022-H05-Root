...
tasks {
    ...
    val graderJar by creating(Jar::class) {
        group = "build"
        afterEvaluate {
            archiveFileName.set("AuD-2022-H05-${project.version}.jar")
    ...
    val graderLibs by creating(Jar::class) {
        ...
        val runtimeDeps = grader.runtimeClasspath.mapNotNull {
            if (it.path.toLowerCase().contains("h05") || jagrRuntime.contains(it)) {
                null
        ...
        from(runtimeDeps)
        archiveFileName.set("AuD-2022-H05-${project.version}-libs.jar")
    }
    create("graderAll") {
        group = "build"
